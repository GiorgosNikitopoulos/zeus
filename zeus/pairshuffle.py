...
class PairShuffle(object):
    ...
    def go_shuffle_prove(self, pi, modulus, order, generator, public, alpha, beta, neff_beta):
        ...
        for i in range(k):
            ...
            self.p1Lamda1 = (self.p1Lamda1 * temporary_variable_2) % modulus #(22)
            temporary_variable_2 = pow(beta[i], temporary_variable, modulus)
            self.p1Lamda2 = (self.p1Lamda2 * temporary_variable_2) % modulus #(23)
        ...
        r = [None] * k
        for i in range(k):
            temporary_variable = (self.v4Zlamda * b[i]) % order #lambda * betai in (27)
        ...
        try:
            return self.pv6.Prove(modulus, order, generator, gamma, r, s) #(30)
        except Exception:
            raise Exception('Error')
    ...
    def go_shuffle_verify(self, modulus, order, generator, public, alpha, beta, alphabar, betabar):
        ...
        for i in range(k):
            ...
            Phi1 = (Phi1 * gmpy2.invert(pow(alpha[i], self.v2Zrho[i], modulus), modulus)) % modulus
            # (32)
            Phi2 = (Phi2 * pow(betabar[i],
                               self.p5Zsigma[i], modulus)) % modulus
            Phi2 = (Phi2 * gmpy2.invert(pow(beta[i], self.v2Zrho[i], modulus), modulus)) % modulus
    ...
    def go_shuffle_shuffle(self, modulus, order, generator, public, alpha, beta):
        ...
        try:
            self.go_shuffle_prove(pi, modulus, order, generator, public, alpha, beta, neff_beta)
        except Exception:
            raise Exception('Error')
        return XBar, YBar
